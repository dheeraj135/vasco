...
public abstract class ForwardInterProceduralAnalysis<M,N,A> extends InterProceduralAnalysis<M,N,A> {
	...
	public void redoAnalysis(Context<M,N,A> c) {
		...
		for (N unit : c.getControlFlowGraph()) {
			c.setValueBefore(unit, topValue());
			c.setValueAfter(unit, topValue());
			c.getWorkList().add(unit);
		}

		for (N unit : c.getControlFlowGraph().getHeads()) {
			c.setValueBefore(unit, copy(c.getEntryValue()));
		}
		...
		while (!worklist.isEmpty()) {
			...
			if (currentContext.getWorkList().isEmpty()) {
				...
				currentContext.removedFromWorklist();
				Cache.freeUp();
				continue;
			}
			...
			if (node != null) {
				...
				if (callers != null) {
					for (CallSite<M,N,A> callSite : callers) {
						...
						callingContext.addedToWorklist();
					}
				}
